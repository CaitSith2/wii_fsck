...
void load_keys_otp(const char *path)
{
	...
	fp = fopen(path, "r");
  if(fp==NULL)
    fatal("Couldn't open otp file %s",path);
	if(fread(otp, 128, 1, fp)!=1)
    fatal("Couldn't read otp data from %s",path);
	fclose(fp);
...
void load_keys_nand_otp(rawnand nand)
{
  char otp[128];
  rawnand_read_otp(nand,otp);
  memcpy(keys.common_key, otp + 20, 16);
	memcpy(keys.nand_key, otp + 88, 16);
	memcpy(keys.nand_hmac, otp + 68, 20);
	get_key("root-key", keys.root_key, sizeof keys.root_key);
